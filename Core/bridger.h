...
struct TraceDetail {
    long long time;           // The time counter when access occurred
    char operation;           // 'R', 'W', 'I'
    uint32_t address;         // The address accessed
    std::string result;       // "HIT", "MISS", "WRITE"
    uint32_t set_index;       // The set index used
};
...
public:
    std::vector<TraceDetail> trace_log;
    ...
    void process_trace_access(const MemoryTrace& trace) {
        ...
        total_accesses++; // Use this as the time counter
        ...
        // 3. LOG THE TRACE DETAIL (ADDED LOGIC)
        TraceDetail detail;
        detail.time = total_accesses;
        detail.operation = trace.operation;
        detail.address = trace.address;
        detail.result = (result == 1) ? "HIT" : "MISS"; // 1 = HIT, 0 = MISS
        detail.set_index = c_addr.index;

        // Add detail to the log
        trace_log.push_back(detail);
        ...
        if (result == 1) {
            total_hits++;
                              ...
                              void print_stats(std::ostream& output_buffer,  const std::string& policy_name){
                                  ...
                                  std::cout << "Total Misses" << total_accesses - total_hits << std::endl;
                                  std::cout << "Overall Miss Rate: " <<(1 - hit_rate) * 100.0 << "%" << std::endl;
                                  std::cout << "Overall Hit Rate: " << hit_rate * 100.0 << "%" << std::endl;

                                  output_buffer<< "\n--- Simulation Summary for " << policy_name << " ---\n";
                                  output_buffer << "Total Accesses: " << total_accesses << std::endl;
                                  output_buffer << "Total Hits:     " << total_hits << std::endl;
                                  output_buffer << "Total Misses" << total_accesses - total_hits << std::endl;
                                  output_buffer << "Overall Miss Rate: " <<(1 - hit_rate) * 100.0 << "%" << std::endl;
                                  output_buffer << "Overall Hit Rate: " << hit_rate * 100.0 << "%" << std::endl;
                              }
};

                          // ----------------------------------------------------------------------

PolicyType get_policy_type(std::string& policy_str) {
...
std::string run_cache_simulator(const std:: string input_file, const std::string policyName, std::string associativity, std::string block_size, std::string seconds){
    ...
    double simulation_period = std::stod(seconds);
    //std::string trace_filename = argv[1];
    std::string policy_str = policyName; //argv[2];
    std::string output_filename = input_file; //argv[1]; // **ADDED**
    ...
    try {
        associativity_val = static_cast<uint32_t>(std::stoul(associativity));//argv[3]
        blockSize_val = static_cast<uint32_t>(std::stoul(block_size));//argv[4]
        modifyAssociativity_blockSize(associativity_val, blockSize_val);
    } catch (const std::exception& e) {
        output_buffer<< "Error: Invalid number format for associativity or block size. They must be integers." << std::endl;
    ...
    if (policy_type == UNKNOWN_POLICY) {
        output_buffer<< "Error: Invalid policy name. Choose from LRU, LFU, or FIFO." << std::endl;
        return output_buffer.str();
    }
    ...
    try {
        ...
        watch.stop();
        simulator.print_stats(output_buffer, get_policy_name(policy_type));

        output_buffer<< "\n---TRACE-LOG-START---\n";

        // Loop through the collected log and print the details line-by-line
        // Format: time,operation,address,result,set_index
        for (const auto& detail : simulator.trace_log) {
            cout << std::dec << detail.time << ","
            << detail.operation << ","
            << "0x" << std::hex << std::uppercase << std::setw(8) << std::setfill('0') << detail.address << ","
            << detail.result << ","
            << std::dec << detail.set_index << "\n";
        }

        for (const auto& detail : simulator.trace_log) {
            output_buffer<< std::dec << detail.time << ","
            << detail.operation << ","
            << "0x" << std::hex << std::uppercase << std::setw(8) << std::setfill('0') << detail.address << ","
            << detail.result << ","
            << std::dec << detail.set_index << "\n";
            ...
            output_buffer<< "Simulation Error: " << e.what() << std::endl;
            return "Error: " + output_buffer.str();
    }
    return output_buffer.str();
}
